...
struct WorkerThread {
    ...
    sender: mpsc::Sender<Pin<Box<OperationWithCallback>>>, // Channel to send ops to the worker thread
}

impl WorkerThread {
    pub fn new(worker_thread_func: fn(mpsc::Receiver<Pin<Box<OperationWithCallback>>>)) -> Self {
        let (sender, rx) = mpsc::channel();
        let handle = thread::spawn(move || worker_thread_func(rx));
        Self { handle, sender }
    }

    pub fn send(&self, op_with_output: Pin<Box<OperationWithCallback>>) {
        self.sender
            .send(op_with_output)
...
impl ObjectStoreAdapter {
    ...
    pub fn new(func_for_get_thread: fn(mpsc::Receiver<Pin<Box<OperationWithCallback>>>)) -> Self {
        Self {
