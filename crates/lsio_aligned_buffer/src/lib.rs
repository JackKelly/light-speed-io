...
impl AlignedBuffer {
    ...
    pub(crate) fn new(slice: Range<usize>, align: usize) -> Self {
        ...
        let layout = alloc::Layout::from_size_align(slice.len() + (slice.start % align), align)
    ...
    pub(crate) fn aligned_start_offset(&self) -> usize {
        (self.valid_slice.start / self.layout.align()) * self.layout.align()
    }
    ...
    pub fn as_slice(&self) -> &[u8] {
        unsafe {
            slice::from_raw_parts(
                self.buf.offset(self.valid_slice.start.try_into().unwrap()),
                self.len(),
            )
        }
    }
...
mod tests {
    ...
    fn test_write_and_read() {
        ...
        {
            let ptr1 = aligned_buf1.as_ptr_to_underlying_buf();
            let ptr2 = aligned_buf2.as_ptr_to_underlying_buf();
            unsafe {
                for i in 0..LEN {
